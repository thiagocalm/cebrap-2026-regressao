...
A começar, vamos usar o pacote que o Guilherme nos recomendou na última aula, chamado `skimr`[^1].

[^1]: Para mais informações sobre ele, você pode acessar [esse texto da Curso-R](https://blog.curso-r.com/posts/2022-05-05-skimr/).
...
$$AcessoEsgoto = \beta_0 + \beta_1 \cdot PBF + \epsilon$$ Para estimar, basta rodar:
...
$$AcessoEsgoto = \beta_0 + \beta_1 \cdot PBF + \beta_2 \cdot Informalidade + \epsilon$$ Para estimar, basta rodar:
...
$$AcessoEsgoto = \beta_0 +  \beta_1 \cdot PBF + \beta_2 \cdot EnsinoMedio + \epsilon$$ Para estimar, basta rodar:
...
$$AcessoEsgoto = \beta_0 +  \beta_1 \cdot PBF + \beta_2 \cdot Informalidade + \beta_3 \cdot EnsinoMedio + \epsilon$$ Para estimar, basta rodar:
...
O que esses resultados nos dizem? Retomando a discussão das aulas anteriores, vale ressaltar que devemos os interpretar em termos de uma **associação**. Ou seja, a relação estabelecida entre essas variáveis não pode ser atribuída ao fato de que uma afeta a outra.

Volte aos resultados dos modelos e tente compreender se faz sentido, em termos substantivos (teóricos) o sentido das relações que observamos neles. Podemos discutir isso na próxima aula!
