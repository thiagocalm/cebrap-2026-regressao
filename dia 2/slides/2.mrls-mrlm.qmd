...
Qual a parcela dos estudantes que tiveram nota em matemática acima da média do ENEM dentro do universo de pessoas que participaram do programa?
...
- $e$: evento -- num. estudantes com nota acima da média;
- $n$: observações -- num. estudantes que fizeram o ENEM e participaram do programa.
:::
...
- **Evento**: parcela de estudantes que participaram do ENEM **e** do programa os quais tiveram nota acima da média em matemática.
...
## [Abordagem empírica = Abordagem probabilística]{.theme-subtopic} {.smaller}

::: nonincremental
Sob o pressuposto de que nossa amostra de estudantes que participaram do ENEM e do programa é **i.i.d.**[^1], a abordagem empírica levaria a resultados *exatamente* iguais ao cálculo via abordagem probabilística!!!
...
[^1]: *i.i.d.*: idêntica e igualmente distribuída. Diz sobre o processo gerador dos nossos dados.

##  {.smaller}

### Relação entre modelos probabilísticos e modelos de regressão

Conhecendo o modelo probabilístico que melhor descreve determinada **variável de interesse**, podemos entender suas propriedades, uma vez que são as mesmas propriedades do modelo probabilístico.

Com isso, podemos:

- **Predição**: Prever resultados futuros condicionados a um conjunto de outras variáveis (probabilidade condicional a um conjunto de $X$);

- **Associação/Causalidade**: Estabelecer em que medida variando uma outra variável(is), os valores da variável de interesse também variam (probabilidade condicional a um $X$).
...
Tudo isso é o que fazemos nos modelos de regressão, graças às propriedades dos modelos probabilísticos subjacentes à variável que estamos interessados.
...
## [Variável aleatória (v.a.)]{.theme-subtopic} {.smaller}
...
Se uma determinada variável é aleatória e é conhecido o seu modelo de probabilidade, a sua *variância* -- ou sua variabilidade em torno da média -- também pode ser conhecida.
...
Conhecendo-se as propriedades de uma variável aleatória de interesse, pode-se relacionar duas variáveis aleatórias $X$ e $Y$, de modo que:
...
Podemos ainda estender essa relação para mais variáveis aleatórias.

##  {.smaller}

### Relação entre variáveis aleatórias

Estendendo a mesma relação:
...
Portanto, tudo o que precisamos saber é qual o modelo probabilístico -- $f(\cdot)$ -- se associa a determinado fenômeno social $Y$, que o trataremos como uma $v.a.$

A partir disso, podemos relacionar esse fenômeno social de interesse com outros fenômenos -- outras $v.a.$
...
Há muitos casos em que estamos interessados em uma -- ou um conjunto de -- variável(is) independente(s). Todavia, por razões das propriedades dos modelos e questões teóricas, é importante que insiramos outras variáveis na análise.

Estas são chamadas de **variáveis controle/confundidoras/co-variáveis**[^2].

[^2]: *Em geral*, seus resultados não são explicados, mas são explicitados.
...
Como a variação de uma determinada variável (independente) afeta a variação de uma outra variável (resposta)?

No caso simplificado do modelo RLS, temos as seguintes características:

- Uma *v.a.* resposta/dependente de **natureza contínua**: $Y$

- Uma *v.a.* explicativa: $X$

- Um fator de erro do modelo que nos diz quanto da variabilidade de $Y$ que continua não explicado por $X$: $\epsilon$

## [RLS: Exemplo de ontem]{.theme-subtopic} {.smaller}
...
# Observacao de 2005 para mostrar o erro

obs <- base |>
  filter(anos == 2004) |>
  mutate(y_hat = predict(modelo, newdata = cur_data()),
         erro = divorcio - y_hat)

# Dados para ilustrar beta 1
x1 <- 4
x2 <- 5
beta1_df <- tibble(x = c(x1, x2), y = beta_0 + beta_1 * c(x1, x2))

# ponto médio para posicionar o texto
beta1_label <- tibble(x = mean(c(x1, x2)) * 1.1,
                      y = mean(beta1_df$y) * 0.95,
                      label = paste0( "\u03B2\u2081 = ",
                                      round(beta_1, 2)) ) # Dados para ilustrar beta 0
beta0_df <- tibble(x = 0, y = c(0, beta_0) )
beta0_label <- tibble(x = 0.25,
                      y = 3.5,
                      label = paste0( "\u03B2\u2080 = ",
                                      round(beta_0, 2) ) )

# Dados para o erro/residuo
erro_label <- obs |>
  mutate( x_label = margarina + 0.15, y_label = (divorcio + y_hat) / 2 )

# GRÁFICO
base |> 
  ggplot(aes(x = margarina, y = divorcio)) + 
  geom_point( size = 4, alpha = .5 ) + 
  geom_smooth(method = "lm", se = FALSE, color = "red4", linewidth = 1.2 ) +
  geom_segment(data = beta0_df,
               aes( x = x, xend = x, y = y, yend = lead(y) ),
               color = "steelblue", linewidth = 1.3, linetype = "dashed" ) +
  geom_point(data = tibble( x = 0, y = beta_0 ),
             aes(x = x, y = y),
             color = "steelblue", size = 4 ) +
  geom_text( data = beta0_label,
             aes( x = x, y = y, label = label ),
             color = "steelblue", fontface = "bold", hjust = 0, size = 5 ) +
  geom_segment( data = beta1_df,
                aes( x = x[1], xend = x[2], y = y[1], yend = y[1] ),
                color = "darkgreen", linewidth = 1.1, linetype = "dashed" ) + 
  geom_segment( data = beta1_df, 
                aes( x = x[2], xend = x[2], y = y[1], yend = y[2] ),
                color = "darkgreen", linewidth = 1.1, linetype = "dashed" ) +
  geom_point( data = beta1_df, 
              aes(x = x, y = y), color = "darkgreen", size = 3 ) +
  geom_text( data = beta1_label,
             aes( x = x, y = y, label = label ),
             color = "darkgreen", 
             fontface = "bold", size = 5, hjust = -0.1, vjust = -0.5 ) +
  geom_segment( data = obs,
                aes(x = margarina,xend = margarina,y = divorcio, yend = y_hat ),
                color = "purple4", linewidth = 1.3, linetype = "dashed" ) +
  geom_point(data = obs,
             aes(x = margarina, y = divorcio), color = "purple4", size = 5 ) +
  geom_point( data = obs,
              aes(x = margarina, y = y_hat),
              color = "purple4", size = 3, shape = 1, stroke = 1.3 ) + 
  geom_text( data = erro_label, 
             aes( x = x_label, y = y_label, label = "eᵢ = yᵢ − ŷᵢ" ), 
             color = "purple4", fontface = "bold", hjust = 0, size = 5 ) +
  labs( x = label_margarina,
        y = label_divorcio, 
        title = "Regressão linear simples",
        subtitle = "Interpretação geométrica dos parâmetros e do erro", caption = "Fonte: Tyler Vigen — Spurious Correlations" ) + 
  coord_cartesian( xlim = c(0, 9), ylim = c(3.5, 5.3), clip = "off" ) + 
  theme_classic() + 
  theme(legend.position = "none",
        plot.title = element_text( face = "bold", size = 16 ),
        plot.subtitle = element_text( color = "grey40", size = 12 ),
        axis.title = element_text( color = "grey33", face = "bold" ),
...
Partindo do nosso modelo RLS para a população, $$Y_i = \beta_0 + \beta_1 \cdot X_i + \epsilon_i$$
...
Modelo populacional: $$Y_i = \beta_0 + \beta_1 \cdot X_i + \epsilon_i$$
...
Os únicos valores que não conhecemos são referentes a $\beta_0$ e $\beta_1$, uma vez que $\epsilon$ é obtido residualmente.
...
- Valores preditos: $\hat{Y_i} = \hat{\beta_0} + \hat{\beta_1} \cdot X_i$ para cada observação $i = 1,...,n$.

- Resíduos: $\hat{u_i}=y_i-\hat{Y_i}$, para cada observação $i=1,...,n$
...
- Exemplo: desigualdades salariais.
...
**Não é que** $\epsilon$ não se relacione com $Y$, e sim que $\epsilon$ não está condicionado a $X$.

##  {.smaller}

### H5: Variância do erro constante para todas as variáveis explicativas

$$Var[\epsilon|X]=\sigma^2$$ Chamamos essa hipótese de **homocedasticidade** ou **variância constante do erro**.
...
- $k$ é o número de parâmetros associados às variáveis explicativas/independentes incluídas no modelo.
